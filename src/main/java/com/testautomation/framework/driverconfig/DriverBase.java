...
public class DriverBase extends ConfigBase {
    ...
    public void handleAlert(String sAcceptOrDecline){
        try {
            ...
            if (wait.until(ExpectedConditions.alertIsPresent()) == null) {
                ...
                if (sAcceptOrDecline.equals(GlobalConstants.C_ALERT_ACCEPT)) {
                    alert.accept();
                } else if (sAcceptOrDecline.equals(GlobalConstants.C_ALERT_DECLINE)) {
                    alert.dismiss();
                }
            }
        } catch (Exception e) {
    ...
    public boolean isExist(WebElement webElement){
        ...
        try{
            element = waitForVisibility(webElement,GlobalConstants.MEDIUM_TIMEOUT);
            if(element!=null){
                elmExist=true;
            }
        }catch (Exception e) {
     ...
     * @return
     */
    public void keyenter(WebElement webElement){
        try{
            webDriverWait = new WebDriverWait(driver, DEFAULT_TIMEOUT);
            if (webDriverWait.until(ExpectedConditions.visibilityOf(webElement)) != null) {
    ...
    public boolean setValueToinputBox(WebElement wTextBoxName,String sText){
        ...
        try {
            ...
            result=verifyInputBoxText(wTextBoxName,sText,GlobalConstants.VERIFY_TEXT_PARTIAL);
        } catch (Exception e) {
    ...
    public boolean checkUncheckCheckBox(WebElement wCheckBox,String checkstatus){
        boolean result = false;
        try {
            if (wCheckBox.isSelected() && checkstatus.equalsIgnoreCase(GlobalConstants.C_CHECK)){
                result=true;
            } else if(wCheckBox.isSelected()==false && checkstatus.equalsIgnoreCase(GlobalConstants.C_CHECK)){
                // Check check box
                wCheckBox.click();
                result = true;
            } else if(wCheckBox.isSelected()==false && checkstatus.equalsIgnoreCase(GlobalConstants.C_UNCHECK)){
                result = true;
            } else if (wCheckBox.isSelected() && checkstatus.equalsIgnoreCase(GlobalConstants.C_UNCHECK)) {
                // UnCheck check box
                wCheckBox.click();
                result = true;
            }
...
//            }
    ...
    public boolean verifyAlertTextMessage(WebElement wLabelName,String sMessageTextToVerify){
        boolean result=true;
        try{
            result=verifyLabelText(wLabelName,sMessageTextToVerify,GlobalConstants.VERIFY_TEXT_PARTIAL);
        }catch (Exception e) {
            result=false;
    ...
    public boolean waitForInVisibility(WebElement element){
        return new WebDriverWait(driver, GlobalConstants.MEDIUM_TIMEOUT).until(ExpectedConditions.invisibilityOf(element));
    }
    ...
    public WebElement waitForVisibility(WebElement element){
        element = new WebDriverWait(driver,GlobalConstants.MEDIUM_TIMEOUT).until(ExpectedConditions.visibilityOf(element));
        return element;
    }
    ...
    public List<WebElement> waitForVisibility(List<WebElement> elements){
        elements = new WebDriverWait(driver,GlobalConstants.MEDIUM_TIMEOUT).until(ExpectedConditions.visibilityOfAllElements(elements));
        return elements;
    }
    ...
    public static WebElement waitForClickability(WebDriver driver, WebElement element){
        waitUntilElementIsDisplayed(element);
        element = new WebDriverWait(driver,GlobalConstants.MEDIUM_TIMEOUT).until(ExpectedConditions.elementToBeClickable(element));
        return element;
    }
